...
contract GuessVerifier {
    ...
    uint256 constant Qmx = 820986266266210090740422826555546466104329529676776159810971499783359517220;
    uint256 constant Qmy = 20374208115418578019566857701905977317074875283659074743261186056769177550942;
    uint256 constant Qlx = 14492873181203848282703380809839994938159205882199181084134239336087111384245;
    uint256 constant Qly = 10267675675115841960857084304001440563333299930911022789583146713704721461370;
    uint256 constant Qrx = 2658765422060733572922833194472485389390637310959106451765924640633654753511;
    uint256 constant Qry = 19469465972771428760372481510972200967302785591692026687200026824935257988244;
    uint256 constant Qox = 14579450164113081573813417496356123455316171389829533668419515685900655853931;
    uint256 constant Qoy = 9098675638735167824120190075895013641877998875600066299019217313035762765627;
    uint256 constant Qcx = 14180155619655033682140442352049336010942246621917319573951995577900033315331;
    uint256 constant Qcy = 179662727306846695013287739580280459331354221813271413264360539865215142378;
    uint256 constant S1x = 10979827263417985205738718237302153389850639544936701248102538773415362608800;
    uint256 constant S1y = 10307529023578691205014431159501849200300131595430955143110138249832383894581;
    uint256 constant S2x = 17270741137373270844340045152573924559522413437162380119433867528136331574179;
    uint256 constant S2y = 6827403353388223246319434543055093606470497440391110474903632184724136580202;
    uint256 constant S3x = 20040187451876312707243565867154755556881996154568075524714935667779625390414;
    uint256 constant S3y = 11140122600946314393282999768382734681824361020604231305122923600323215849191;
    ...
    function verifyProof(bytes memory proof, uint[] memory pubSignals) public view returns (bool) {
        assembly {
            ...
            function calculateChallanges(pProof, pMem, pPublic) {
                ...
                mstore( add(pMem, 1024 ), mload( add( pPublic, 32)))
                
                mstore( add(pMem, 1056 ), mload( add( pPublic, 64)))
                
                mstore( add(pMem, 1088 ), mload( add( pPublic, 96)))
                
                mstore( add(pMem, 1120 ), mload( add( pPublic, 128)))
                
                mstore( add(pMem, 1152 ), mload( add( pPublic, 160)))
                
                mstore( add(pMem, 1184 ), mload( add( pPublic, 192)))
                
                mstore( add(pMem, 1216 ), mload( add( pPublic, 224)))
                
                mstore( add(pMem, 1248 ), mload( add( pPublic, 256)))
                
                mstore( add(pMem, 1280 ), mload( add( pPublic, 288)))
                
                mstore( add(pMem, 1312 ), mload( add( pPublic, 320)))
                
                mstore( add(pMem, 1344 ), mload( add( pPublic, 352)))
                
                mstore( add(pMem, 1376 ), mload( add( pProof, pA)))
                mstore( add(pMem, 1408 ), mload( add( pProof, add(pA,32))))
                mstore( add(pMem, 1440 ), mload( add( pProof, add(pA,64))))
                mstore( add(pMem, 1472 ), mload( add( pProof, add(pA,96))))
                mstore( add(pMem, 1504 ), mload( add( pProof, add(pA,128))))
                mstore( add(pMem, 1536 ), mload( add( pProof, add(pA,160))))
                
                b := mod(keccak256(add(pMem, lastMem), 544), q) 
